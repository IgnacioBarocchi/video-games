...
export const NPCComposition: FC<NPCCompositionProps> = memo(
  ({
    ...
    const inactivateNPCsPosition: [number, number, number] = useMemo(
      () => [0, 0, -gap * numberOfNPCs - triggerGap],
      [gap, numberOfNPCs, triggerGap]
    );
    ...
    const handleMount = (payload: IntersectionExitPayload) => {
    ...
    const handleUnmount = (payload: IntersectionExitPayload) => {
      if (payload?.other?.rigidBodyObject?.name !== ENTITY.CAR) return;
    ...
    return (
      <group position={position}>
        ...
        <Sensors
          ...
          activateEntitiesPosition={activateNPCsPosition}
          inactivateEntitiesPosition={inactivateNPCsPosition}
        />
      </group>
    );
  }
